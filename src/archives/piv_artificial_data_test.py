...
def main():
    ...
    new_points = np.random.choice(tmp, (80,3))
    ...
    for i in range(3, m):
        ...
        K2is = build_constraints_matrix(img2_points[0], img2_points[1], img2_points[2], proj_depth2[0],
                                        proj_depth2[1], proj_depth2[2], img2_points[i], proj_depth2[i])
        K3is = build_constraints_matrix(img3_points[0], img3_points[1], img3_points[2], proj_depth3[0],
                                        proj_depth3[1], proj_depth3[2], img3_points[i], proj_depth3[i])
        K4is = build_constraints_matrix(img4_points[0], img4_points[1], img4_points[2], proj_depth4[0],
        ...
        expected = np.array([uv, vv, proj_depth_v[i], 1, 1])
        ...
        b = [[0,0,0,-np.inf,-np.inf], [IMAGE_SIZE,IMAGE_SIZE,600,np.inf,np.inf]]
        ...
        res = minimize(sum_of_squares_of_F_3var, x0, cst, method='L-BFGS-B', bounds=b, constraints=cons)
        ...
        error_depth = np.append(error_depth, abs(proj_depth_v[i] - d))     
    ...
    print("Nb of bad reconstruction and high residual: ", cnt_bad, cnt_high_res)
